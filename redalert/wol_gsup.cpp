...
extern unsigned PlanetWestwoodStartTime; //Time that game was started
...
RESULT_WOLGSUP WOL_GameSetupDialog::Show()
{
    ...
    if (Color == PCOLOR_NONE)
        SetPlayerColor(pWO->szMyName,
                       ColorNextAvailable()); //	Unless I'm host, this will be changed quite immediately,
...
*/
        ...
        case (BUTTON_SCENARIOLIST | KN_BUTTON): {
            if (pILScens->Count()) {
                int iSelectedScenIndex = (int)(intptr_t)pILScens->Get_Item_ExtraDataPtr(pILScens->Current_Index());
...
void WOL_GameSetupDialog::ScenarioDisplayMode(SCENARIO_GAMEKIND ScenKind)
{
    ...
    for (int i = 0; i != ar_szScenarios[ScenKind].Count(); i++) {
        ...
        int iScenIndex = (int)(intptr_t)ar_szScenIndexes[ScenKind][i];
...
void WOL_GameSetupDialog::ProcessGuestRequest(User* pUser, const char* szRequest)
{
    ...
    case WOL_GAMEOPT_REQSTART:
        //	Does Param ID of accept request match the last param change ID sent? See notes at top.
        if (atoi(szRequest)
            == nHostLastParamID) //	Otherwise ignore - it's old and we don't care. (Incredibly unlikely to happen, actually.)
        {
...
//***********************************************************************************************
//	bSpecialAftermathScenario() now lives in netdlg.cpp.
extern bool bSpecialAftermathScenario(const char* szScenarioDescription);
