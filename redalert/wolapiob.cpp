    ...
    , bShowRankUpdated(false)
{
    ...
    *szGameResServerHost2 = 0;
    ...
    //	The system browser is opened through SDL; no executable lookup needed.
    *szWebBrowser = 0;
}
...
{
    //	Create the lobby client objects and attach our event sinks.
    //	(Replaces the COM connection-point setup against wolapi.dll.)
    pChatSink = new RAChatEventSink(this);
    pChatSink->AddRef();
    pNetUtilSink = new RANetUtilEventSink(this);
    pNetUtilSink->AddRef();
    if (!WOL_Create_Clients(&pChat, &pNetUtil, pChatSink, pNetUtilSink)) {
        return false;
    }
    return true;
}
...
{
    //	Release the lobby client objects and our sinks.
    if (pChat) {
        pChat->Release();
        pChat = NULL;
    }
...
void WolapiObject::PrepareButtonsAndIcons()
{
    ...
    WOLPALETTE hPal = GetCurrentScreenPalette();
...
//***********************************************************************************************
void WolapiObject::GetGameTypeInfo(int iGameType, WOL_GAMETYPEINFO& GameTypeInfo, WOLPALETTE hPal)
{
...
else
{
	...
	pChatSink->pServer = new WOLServer;
...
void WolapiObject::DoFindPage()
{
    ...
    Fancy_Text_Print(TXT_NONE, 0, 0, GadgetClass::Get_Color_Scheme(), TBLACK, TPF_TEXT); //	Required before String_Pixel_Width() call, for god's sake.
...
{
    //	Accounts are created by the lobby service itself: logging in with a
    //	nickname that does not exist yet registers it. Nothing to launch.
    WWMessageBox().Process(TXT_WOL_WEBREGISTRATIONSHELL);
    return false;
}
...
{
    //	Hand the URL to the system browser. The game keeps running.
    if (szURL == NULL || *szURL == 0) {
        return false;
    }
#if defined(SDL2_BUILD) && SDL_VERSION_ATLEAST(2, 0, 14)
    if (SDL_OpenURL(szURL) == 0) {
        return true;
    }
#endif
    WWMessageBox().Process(TXT_WOL_CANTLAUNCHBROWSER);
    return false;
}
...
bool WolapiObject::RequestIPs(const char* szName)
{
    ...
    pChatSink
        ->DeleteUserIPList(); //	Clear old user IPs. (To keep searches fast, if we go in and out of game channels a lot.)
...
{
    //	Get stored defaults for options from the game's settings file.
    INIClass ini;
    CCFileClass file(CONFIG_FILE_NAME);
    if (file.Is_Available()) {
        ini.Load(file);
    }
    bFindEnabled = ini.Get_Bool("WOL", "FindEnabled", true);
    bPageEnabled = ini.Get_Bool("WOL", "PageEnabled", true);
    bLangFilter = ini.Get_Bool("WOL", "LangFilter", true);
    bAllGamesShown = ini.Get_Bool("WOL", "ShowAllGames", true);
...
{
    //	Set options and remember them in the game's settings file.
    ...
    ini.Put_Bool("WOL", "FindEnabled", bFindEnabled);
    ini.Put_Bool("WOL", "PageEnabled", bPageEnabled);
    ini.Put_Bool("WOL", "LangFilter", bLangFilter);
    ini.Put_Bool("WOL", "ShowAllGames", bAllGamesShown);
    ini.Save(file);
    pChat->SetFindPage(bFindEnabled, bPageEnabled);
    pChat->SetLangFilter(bLangFilter);
}
...
//***********************************************************************************************
WOLPALETTE GetCurrentScreenPalette()
{
    //	The palette the lobby screens are drawn with.
    return &GamePalette;
}

//***********************************************************************************************
void RemapDIBToPalette(WOLPALETTE hPal, const char* pDIB) //	Note: pDIB is treated as non-const.
{
    //	Converts pDIB's pixel data to the nearest entries of the game palette.
    ...
    RGBQUAD* pRGBEntry = (RGBQUAD*)((char*)lpbi + lpbi->biSize); //	First entry in the bmp's palette table.
    qNewPalette[0] = (unsigned char)hPal->Closest_Color(RGBClass(0, 0, 0));
    pRGBEntry++;
    for (int i = 1; i != 256; i++) {
        qNewPalette[i] = (unsigned char)hPal->Closest_Color(RGBClass(pRGBEntry->rgbRed, pRGBEntry->rgbGreen, pRGBEntry->rgbBlue));
        pRGBEntry++;
    }
